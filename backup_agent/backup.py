...
class BackupAgent:
    ...
    def get_last_backup_folder(self, usb: Path) -> Path | None:
        ...
        latest = max(folders, key=count_files)
        return latest
    
    def file_exists_in_backup(self, file_rel: Path, last_backup: Path | None) -> tuple[bool, float]:
        if last_backup is None:
            return False, 0
        
        backup_file = last_backup / file_rel
        if not backup_file.exists():
            return False, 0
        
        try:
            backup_mtime = backup_file.stat().st_mtime
            return True, backup_mtime
        except OSError:
            return False, 0
    ...
    def get_files_to_backup(self, last_backup: Path | None) -> list[tuple[Path, Path]]:
        ...
        for folder in source_folders:
            ...
            for file in folder.rglob("*"):
                ...
                exists, backup_mtime = self.file_exists_in_backup(rel, last_backup)
                
                if exists:
                    try:
                        current_mtime = file.stat().st_mtime
                        if current_mtime <= backup_mtime + 2:
                            self.stats["skipped_unchanged"] += 1
                            continue
                    except OSError:
                        pass
                
                files_to_backup.append((file, rel))
    ...
    def run_backup(self, usb: Path, dry_run: bool = False):
        ...
        last_backup = self.get_last_backup_folder(usb)
        
        if last_backup:
            self.logger.info(f"Last backup: {last_backup.name}")
            print(f"\nLast backup: {last_backup.name}")
        else:
            self.logger.info("No previous backup found - backing up all files")
            print("\nNo previous backup found - backing up all files")
        
        files_to_backup = self.get_files_to_backup(last_backup)
        
        if not files_to_backup:
            self.logger.info("No modified files to backup")
            print("No modified files to backup")
            return
        
        total = len(files_to_backup)
        self.logger.info(f"Found {total} modified files to backup")
        print(f"Found {total} modified files to backup")
        
        for idx, (file, rel) in enumerate(files_to_backup):
            dest = dest_root / rel
